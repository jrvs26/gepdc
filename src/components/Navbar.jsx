...
function Navbar() {
    ...
    useEffect(() => {
        if (document.title !== "Global Electric Power Devt Corp" && document.title !== "About GEPDC") {
            setIsSticky(true);  // Set sticky navbar if title is not the specific one
        } else {
            setIsSticky(false); // set sticky navbar if title is "Global Electric Power Devt Corp"
        }
    }, []); // Empty dependency array ensures this runs only once after component mounts

    return (
        <header
            className={`font-[sans-serif] tracking-wide ${isSticky ? 'fixed top-0 left-0 right-0 z-50 bg-white bg-opacity-40 backdrop-blur-md' : ''}`}>
            {/* Main navbar section */}
                ...
                <div id="collapseMenu" className={`lg:!block ${isMenuOpen ? 'block' : 'hidden'}`}>
                    ...
                    <ul className={`lg:flex lg:gap-x-10 max-lg:space-y-3 max-lg:fixed max-lg:bg-white max-lg:w-full
                        ...
                        <li className='group max-lg:border-b max-lg:px-3 max-lg:py-3 relative'>
                            <Link to='/about' className='hover:text-green-500 text-gray-700 font-semibold text-[15px] block'>
                                About
                            ...
                            <ul className='absolute top-5 max-lg:top-8 left-0 z-50 rounded-xl block space-y-2 shadow-lg bg-white max-h-0 overflow-hidden min-w-[200px] group-hover:opacity-100 group-hover:max-h-[700px] px-6 group-hover:pb-4 group-hover:pt-6 transition-all duration-500'>
                                <li className='border-b pb-2'>
                                    <Link to='/about' className='hover:text-green-500 text-gray-600 font-semibold text-[15px] block'>About Our Company</Link>
                                </li>
                        ...
                        <li className='group max-lg:border-b max-lg:px-3 max-lg:py-3 relative'>
                            <Link to='/projects' className='hover:text-green-500 text-gray-700 font-semibold text-[15px] block'>
                                Projects
                                <svg xmlns="http://www.w3.org/2000/svg" width="16px" height="16px" className="ml-1 inline-block" viewBox="0 0 24 24">
                                    <path d="M12 16a1 1 0 0 1-.71-.29l-6-6a1 1 0 0 1 1.42-1.42l5.29 5.3 5.29-5.29a1 1 0 0 1 1.41 1.41l-6 6a1 1 0 0 1-.7.29z" />
                                </svg>
                            ...
                            <ul className='absolute top-5 max-lg:top-8 left-0 z-50 block rounded-xl space-y-2 shadow-lg bg-white max-h-0 overflow-hidden min-w-[260px] group-hover:opacity-100 group-hover:max-h-[700px] px-6 group-hover:pb-4 group-hover:pt-6 transition-all duration-500'>
                                <li className='border-b pb-2'>
                                    <Link to='/projects/giga8' className='hover:text-green-500 text-gray-600 font-semibold text-[15px] block'>GIGA 8 Project</Link>
                                </li>
                                <li className='border-b py-3'>
                                    <Link to='/projects/sspc' className='hover:text-green-500 text-gray-600 font-semibold text-[15px] block'>SSPC Project</Link>
                                </li>
                                <li className='border-b py-3'>
                                    <Link to='/projects/dspp' className='hover:text-green-500 text-gray-600 font-semibold text-[15px] block'>DSPP Project</Link>
                                </li>
                                <li className='pt-2'>
                                    <Link to='/projects/tera3' className='text-gray-400 font-semibold text-[15px] block cursor-not-allowed' aria-disabled='true'>
                                        TERA 3 Project
                        ...
                        <li className='group max-lg:border-b max-lg:px-3 max-lg:py-3 relative'>
                            ...
                            <ul className='absolute top-5 max-lg:top-8 left-0 z-50 block rounded-xl space-y-2 shadow-lg bg-white max-h-0 overflow-hidden min-w-[250px] group-hover:opacity-100 group-hover:max-h-[700px] px-6 group-hover:pb-4 group-hover:pt-6 transition-all duration-500'>
                                <li className='border-b pb-2'>
                                    <Link to='/services/solar-plant' className='hover:text-green-500 text-gray-600 font-semibold text-[15px] block'>Solar Power Plant</Link>
                                </li>
                                <li className='border-b py-3'>
                                    <Link to='/services/thermal-scanning' className='hover:text-green-500 text-gray-600 font-semibold text-[15px] block'>Thermal Scanning</Link>
                                </li>
                            </ul>
                        </li>
                        ...
                        {/* Contact Link */}
                        <li className='border-b py-3'>
                            <Link to='/contact' className='hover:text-green-500 text-gray-700 font-semibold text-[15px] block'>
                                Contact
                            </Link>
                        </li>
                    </ul>
